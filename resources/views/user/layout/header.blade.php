...
<header
    ...
    <div x-data="{ showMessage: false }" class="relative inline-block">
        <button class="text-lg font-semibold text-gray-800 flex items-center" @click="showMessage = !showMessage" type="button" aria-haspopup="true" :aria-expanded="showMessage.toString()">
                ...
                @foreach ($notifications as $notification)
                    ...
                    >
                        ...
                        @if (isset($notification->data['url']))
                            <a href="{{ $notification->data['url'] }}" class="text-blue-600 hover:underline mt-1 inline-block text-xs">
                                View Booking
                            </a>
                        @endif
                        @if (!$notification->read_at)
                            <form method="POST" action="{{ route('user.notifications.markRead', $notification->id) }}" class="inline ml-2">
                                @csrf
